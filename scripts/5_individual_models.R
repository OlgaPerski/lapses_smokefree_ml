...
rf_sample <- df %>%
  ungroup() %>%
  filter(account_id %in% lapse_count) %>%
  group_by(account_id) %>%
  group_split()

rf_sample <- lapply(rf_sample, function(x) x %>% select(-c("account_id", 
                                                           "cigs_per_day", 
                                                           "time_to_first_cig", 
                                                           "days_since_qd", 
                                                           "mins_since_prev_rep"))) # remove vars with zero variance (baseline characteristics)
...
run_rf_models <- function(x) {
  ...
  data_split <- initial_split(x, prop = .80, strata = craving_did_smoke)
...
run_svm_models <- function(x) {
  ...
  svm_rec <- recipe(craving_did_smoke ~ ., data = train_data) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
    step_zv(all_predictors()) %>%
    step_normalize(craving_severity) %>%
    step_upsample(craving_did_smoke, skip = TRUE) # manages class imbalances
...
run_elnet_models <- function(x) {
  ...
  elnet_rec <- recipe(craving_did_smoke ~ ., data = train_data) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
    step_zv(all_predictors()) %>%
    step_normalize(craving_severity) %>%
    step_upsample(craving_did_smoke, skip = TRUE) # manages class imbalances
...
run_xgb_models <- function(x) {
  ...
  xgb_rec <- recipe(craving_did_smoke ~ ., data = train_data) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
    step_zv(all_predictors()) %>%
    step_normalize(craving_severity) %>%
    step_upsample(craving_did_smoke, skip = TRUE) # manages class imbalances
...
if(!file.exists(here("outputs", "individual_model_performance_plot.png"))) ggsave(individual_model_performance_plot, 
                                                                                  filename = here("outputs", "individual_model_performance_plot.png"),
...
if(!file.exists(here("outputs", "individual_best_model_performance_stratified_plot.png"))) ggsave(individual_best_model_performance_stratified_plot, 
                                                                                  filename = 
                                                                                    here("outputs", "individual_best_model_performance_stratified_plot.png"),
                                                                                  dpi = 320, height = 8, width = 10)
...
individual_vip_by_model_type <- extract_model_vip() %>%
  ...
  ungroup() %>%
  mutate(variable = str_remove(variable, "craving"),
         ...
         variable = str_replace_all(variable, "time of day", "Time of day:"),
         ...
         variable = str_replace_all(variable, "3", "Wednesday"),
         variable = str_replace_all(variable, "4", "Thursday"),
         variable = str_replace_all(variable, "5", "Friday"),
         variable = str_replace_all(variable, "6", "Saturday"),
         model = str_to_upper(model),
         group = factor(case_when(str_detect(variable, "Feeling|Craving") ~ "Feeling state",
                           str_detect(variable, "Activity") ~ "Activity",
  ...
  ggplot() +
  geom_point(aes(x = fct_rev(variable), y = model, size = prop_n, colour = group)) +
...
if(!file.exists(here("outputs", "individual_vip_by_model_type.png"))) ggsave(individual_vip_by_model_type, 
                                                                             filename = here("outputs", "individual_vip_by_model_type.png"), 
                                                                             dpi = 320, height = 8, width = 10)
