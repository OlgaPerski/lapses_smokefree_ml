...
exclude <- c("account_id", "adjusted_quit_date", "adjusted_craving_record_created", 
             ...
             "event_nr", "n", "lapse_events", "prop_lapses", "prior_event_lapse")
...
vip_rf_sensitivity_no_prior_lapse <- final_rf %>%
  set_engine("ranger", importance = "permutation") %>%
  fit(craving_did_smoke ~ .,
      data = juice(rf_prep)) %>%
  vip()

if(!file.exists(here("outputs", "vip_rf_sensitivity_no_prior_lapse.png"))) ggsave(vip_rf_sensitivity_no_prior_lapse, 
                                                                                  filename = here("outputs", "vip_rf_sensitivity_no_prior_lapse.png"), 
...
vip_svm_sensitivity_no_prior_lapse <- svm_fit %>%
  extract_fit_parsnip() %>%
  vip(method = "permute", 
      target = "craving_did_smoke", metric = "auc", reference_class = "true",
      pred_wrapper = kernlab::predict, train = juice(rf_prep))

if(!file.exists(here("outputs", "vip_svm_sensitivity_no_prior_lapse.png"))) ggsave(vip_svm_sensitivity_no_prior_lapse, 
                                                                                   filename = here("outputs", "vip_svm_sensitivity_no_prior_lapse.png"), 
                                                                                   dpi = 320, height = 8, width = 10)

# elastic net

vip_elnet_sensitivity_no_prior_lapse <- final_elnet %>%
  set_engine("glmnet", importance = "permutation") %>%
  fit(craving_did_smoke ~ .,
      data = juice(rf_prep)) %>%
  vip()

if(!file.exists(here("outputs", "vip_elnet_sensitivity_no_prior_lapse.png"))) ggsave(vip_elnet_sensitivity_no_prior_lapse, 
                                                                                     filename = here("outputs", "vip_elnet_sensitivity_no_prior_lapse.png"), 
                                                                                     dpi = 320, height = 8, width = 10)

# xgboost

vip_xgb_sensitivity_no_prior_lapse <- final_xgb %>%
  set_engine("xgboost") %>%
  fit(craving_did_smoke ~ .,
      data = juice(rf_prep)) %>%
  vip()

if(!file.exists(here("outputs", "vip_xgb_sensitivity_no_prior_lapse.png"))) ggsave(vip_xgb_sensitivity_no_prior_lapse, 
                                                                                   filename = here("outputs", "vip_xgb_sensitivity_no_prior_lapse.png"), 
                                                                                   dpi = 320, height = 8, width = 10)
...
combined_roc_sensitivity_no_prior_lapse <- bind_rows(rf_auc, elnet_auc, svm_auc, xgb_auc) %>%
...
if(!file.exists(here("outputs", "plot_combined_roc_sensitivity_no_prior_lapse.png"))) ggsave(combined_roc_sensitivity_no_prior_lapse, 
                                                                                             filename = here("outputs", "plot_combined_roc_sensitivity_no_prior_lapse.png"), 
                                                                                             dpi = 320, height = 8, width = 10)
